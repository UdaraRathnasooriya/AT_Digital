...
const HorizontalNavBar = () => {
  ...
  return (
    <Fragment>
      ...
      {toggle && (
        <nav className="bg-primary h-screen ss:hidden flex pt-8 border-t-2 justify-center">
          <ul className=" space-y-5 flex flex-col  w-full mx-2  ss:hidden  text-white font-inter">
            {navLinks.map((link) => {
              return (
                <li
                  key={link.id}
                  className="font-medium  rounded-md  text-gray-100 hover:bg-gray-300 hover:text-gray-800 hover:font-bold"
                >
